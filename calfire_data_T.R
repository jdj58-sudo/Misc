...
ggplot() +
  ...
  geom_sf(data = six_rivers, aes(fill = cause_text), color = "grey", alpha = 0.9) +
  coord_sf() +
  #scale_fill_brewer(palette="Set2")
  theme_minimal()
...
srf_c %>%
  ...
  ggplot(aes(x = year, y = acres, fill = cause_text)) +
  geom_col() +
  scale_fill_manual(values = c("darkgreen", "blue", "orange", "purple", "red", "brown", "gray", "black", "gold", "navy", "magenta")
) +  # Customize as needed
  labs(
    title = "Wildfire Causes Over Time",
    x = "Year",
    y = "Acres Burned",
    fill = "Cause"
  ) +
...
#California Fire Perimetters CITATION

######################################################
# filtered years #Use this plot for causes
srf_c %>% as.data.frame %>%
  rename(year = YEAR_,
         acres=GIS_ACRES, 
         cause= cause_text) %>% 
